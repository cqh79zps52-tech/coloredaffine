...
class TrayState implements Disposable {
  ...
  getSecondaryMenuProvider(): TrayMenuProvider {
    return {
      ...
      getConfig: () => [
        {
          label: 'Open ColoredAFFiNE',
          click: () => {
            logger.info('User action: Open ColoredAFFiNE');
            getMainWindow()
              .then(w => {
                w.show();
              })
              .catch(err => {
                logger.error('Failed to open ColoredAFFiNE:', err);
              });
          },
        },
        {
        ...
        'separator',
        {
          label: 'Quit ColoredAFFiNE Completely...',
          click: () => {
            logger.info('User action: Quit ColoredAFFiNE Completely');
            app.quit();
          },
        },
      ],
